...
http://localhost:3333 でリアルタイム戦況を表示。
...
| バージョン | 機能 | 説明 |
|-----------|------|------|
| v1.0 | Markdownレンダリング | dashboard.md → HTML |
| v2.0 | WebSocket更新 | ファイル変更時に自動更新 |
| v3.0 | ゲーム性UI | プログレスバー、足軽カード、リアルタイムログ、XP/レベル/称号、コンボ、勝利演出 |
| v3.1 | スキルパネル | 巻物庫（秘伝書/奥義）、味方軍バフ表示、オーラエフェクト |
| v3.2 | 陣形/統計/アチーブメント | 陣形選択パネル、累計統計、アチーブメント解除（※JS未実装） |

### セルフブラッシュアップ方針

敵 = localhost(dashboard.html)のブラッシュアップ項目として定義。
/shogunでダッシュボード自体を改善する「メタ」体験を実現。

```
dashboard.mdの「進行中」に改善タスクを記載
  ↓
localhost:3333で「敵 = 改善項目」として表示
  ↓
足軽がdashboard.htmlを更新
  ↓
タスク完了 → 「敵撃破」表示
  ↓
ブラウザリロードで新機能が反映される
