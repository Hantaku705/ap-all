...
シャンプー関連SNS投稿のソーシャルリスニング分析。Meltwaterデータ10,000件を可視化。
...
| データ件数 | 10,000件 |
| プラットフォーム | Instagram, Twitter/X, YouTube等 |
| 地域 | 主に日本 |

**現在のステージ**: Stage 1（Input Capture）完了
...
│   └── brief.md            # データ概要・分析方針
├── source/
│   └── export_...csv       # Meltwater生データ
└── webapp/                 # Next.js Webアプリ（未実装）
```
